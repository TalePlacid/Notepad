...
void PageSettingForm::OnPaint() {
	...
	//1. 종이 그림자 영역을 그린다.
	CPen solidPen(PS_SOLID, 1, RGB(0, 0, 0));   // 검은색, 1픽셀 선
	CBrush blackBrush(RGB(0, 0, 0));      // 검은색 채우기
	...
	dc.SelectObject(&whiteBrush); // 흰색 채우기.
	...
	CPen dotPen(PS_DOT, 1, RGB(255, 0, 0)); // 점선, 빨간색.
